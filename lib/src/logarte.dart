...
class Logarte {
  ...
  @Deprecated('Use logarte.log() instead')
  void error(Object? message, {StackTrace? stackTrace, bool write = true}) {
    _log(
      'ERROR: $message\n\nTRACE: $stackTrace',
      write: write,
      trace: Trace.current(),
  ...
  void network({
    ...
    try {
      _log('[${request.method}] URL: ${request.url}', write: write);
      _log('HEADERS: ${request.headers.prettyJson}', write: write);
      _log('BODY: ${request.body.prettyJson}', write: write);
      _log('STATUS CODE: ${response.statusCode}', write: write);
      _log('RESPONSE HEADERS: ${response.headers.prettyJson}', write: write);
      _log('RESPONSE BODY: ${response.body.prettyJson}', write: write);

      _add(NetworkLogarteEntry(request: request, response: response));
    } catch (_) {}
  }
  ...
  void database({
    ...
    try {
      ...
      _add(DatabaseLogarteEntry(target: target, value: value, source: source));
    } catch (_) {}
  }
